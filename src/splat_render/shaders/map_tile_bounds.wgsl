...
@group(0) @binding(2) var<storage> conics: array<vec4f>;
...
@group(0) @binding(8) var<storage, read_write> tile_set_gid: array<atomic<u32>>;
...
fn main(@builtin(global_invocation_id) global_id: vec3u) {
    let idx = global_id.x;
    let num_gaussians = arrayLength(&gassuan_ids_sorted);
    ...
    let bit_index = (idx - bucket * bucket_size) / bucket_bit_size;
    ...
    for (var ty = tile_min.y; ty < tile_max.y; ty++) {
        for (var tx = tile_min.x; tx < tile_max.x; tx++) {
            ...
            if helpers::can_be_visible(tile_start, conic, xy, opac) {
                let tile_bucket_id = tile_id * helpers::BUCKET_COUNT + bucket;

                atomicMin(&tile_min_gid[tile_bucket_id], idx);
                atomicMax(&tile_max_gid[tile_bucket_id], idx + 1u);
                atomicOr(&tile_set_gid[tile_bucket_id], bit_mask);
            }
        }
    }
}
